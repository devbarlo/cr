...
@jepthon.ar_cmd(pattern="tdata")

async def td(event):
    return await edit_or_reply(event, str(t))

@jepthon.ar_cmd(pattern="توب الفلوس(.*)")
...
@jepthon.ar_cmd(pattern="مسح حسابي(.*)")
...
@jepthon.on(admin_cmd(pattern="(فلوسي|اموالي) ?(.*)"))
async def a(message):
    me = await message.client.get_me()
...
@jepthon.on(admin_cmd(pattern="(بنكي|مصرفي) ?(.*)"))
...
@jepthon.ar_cmd(func=lambda m:"راتب")
...
@jepthon.ar_cmd(pattern="انشاء حساب (.*)")
async def bankar(message):
    ...
    if input == "سورس كرستين":
        bankn = "مصرف سورس كرستين "
    elif input == "الرافدين":
    	bankn = "مصرف الرافدين"
    elif input != "الرافدين" or "تليثون كرستين":
