...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("▾∮ هنالك خطأ يرجى تبليغنا @cr_source", str(err))           
...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
@jepthon.on(admin_cmd(pattern="{up_admin} ?(.*)"))
async def gben(jepthon):
    ...
    try:
        ...
        pass
    if me == user:
       jepthon = await razan.edit("▾∮ لا استطيع رفع نفسي 🧸🤍،")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
...
@jepthon.on(admin_cmd(pattern="{down_admin} ?(.*)"))
async def gben(jepthon):
    dc = razan = jepthon
    i = 0
    sender = await dc.get_sender()
    ...
    try:
        ...
        pass
    if me == user:
       jepthon = await razan.edit("▾∮ لا استطيع تنزيل نفسي 🧸🤍")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
