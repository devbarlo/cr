...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 1050898456 or user.id == 1099460779 or user.id == 1001132193 or user.id == 1355571767 or user.id == 929790454 or user.id == 1833610203:
...
@jepthon.on(admin_cmd(pattern="رفع مطي(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    """Generates a link to the user's PM with a custom text."""
    user, custom = await get_user_from_event(mention)
    if not user:
...
@jepthon.on(admin_cmd(pattern="كرستين(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    await edit_or_reply(mention, f"كرستين علي مطور سورس كرستين @dr_criss")
...
async def OwnerStart(event):
    ...
    if sender.id == ownersayed_id :
        order = await event.reply('اهلا بتاج راسنا وحامي عرضنه كرستين - @cr_source')
