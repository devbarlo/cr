        ...
        **kwargs,
        ...
        def decorator(func):  # sourcery no-metrics
            ...
            if pattern is not None:
                ...
                if edited:
                    jepthon.add_event_handler(func, events.MessageEdited(**kwargs))
                jepthon.add_event_handler(func, events.NewMessage(**kwargs))
            return wrapper
    ...
    def bot_cmd(
        ...
        **kwargs,
        ...
        def decorator(func):
                ...
                try:
                    ...
                    if not disable_errors:
                        ...
                        ftext = f"\nDisclaimer:\nThis file is pasted only here ONLY here,\
                                    ...
                                    \nyou may not report this error if you've\
                                    \nany confidential data here, no one will see your data\
                                    \n\n--------BEGIN CristenUserbot TRACEBACK LOG--------\
                                    \nDate: {date}\nGroup ID: {str(check.chat_id)}\
                                    \nSender ID: {str(check.sender_id)}\
                                    \nMessage Link: {await check.client.get_msg_link(check)}\
                                    \n\nEvent Trigger:\n{str(check.text)}\
            ...
            if edited is True:
                jepthon.tgbot.add_event_handler(func, events.MessageEdited(**kwargs))
            else:
                jepthon.tgbot.add_event_handler(func, events.NewMessage(**kwargs))
